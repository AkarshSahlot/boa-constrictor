 ...
 */
  ...
  // Search multiple locations for EnergyLib64.dll
  std::vector<std::string> search_paths;

  // 1. Environment variable override
  const char* env_path = std::getenv("ENERGYLIB64_PATH");
  if (env_path) search_paths.push_back(std::string(env_path));

  // 2. System32
  char tpath[MAX_PATH];
  GetSystemDirectoryA(tpath, MAX_PATH);
  search_paths.push_back(std::string(tpath) + "\\EnergyLib64.dll");

  // 3. Common Intel Power Gadget install locations
  const char* progfiles = std::getenv("ProgramFiles");
  if (progfiles) {
    search_paths.push_back(std::string(progfiles) + "\\Intel\\Power Gadget 3.6\\EnergyLib64.dll");
    search_paths.push_back(std::string(progfiles) + "\\Intel\\Power Gadget\\EnergyLib64.dll");
  }
  const char* progfiles86 = std::getenv("ProgramFiles(x86)");
  if (progfiles86) {
    search_paths.push_back(std::string(progfiles86) + "\\Intel\\Power Gadget 3.6\\EnergyLib64.dll");
    search_paths.push_back(std::string(progfiles86) + "\\Intel\\Power Gadget\\EnergyLib64.dll");
  }

  // 4. Current directory
  search_paths.push_back("EnergyLib64.dll");

  for (const auto& p : search_paths) {
    handler = LoadLibrary(p.c_str());
    if (handler) {
      std::cout << "Loaded EnergyLib64.dll from: " << p << std::endl;
      break;
    }
  }
  ...
  if (!handler)
  {
    std::cout << "Power gadget could not be loaded (EnergyLib64.dll not found)" << std::endl;
    std::cout << "  Set ENERGYLIB64_PATH=<full path to EnergyLib64.dll> or copy it to System32" << std::endl;
    return;
  }
  ...
  for (int i = 0; i < numNodes; i++)
  {
    for (int j = 0; j < numMsrs; j++)
    {
      ...
      if (func == MSR_FUNC_POWER)
      {
        ...
        int len = WideCharToMultiByte(CP_UTF8, 0, namewstr.c_str(), -1, nullptr, 0, nullptr, nullptr);
        std::string namestr(len > 0 ? len - 1 : 0, '\0');
        WideCharToMultiByte(CP_UTF8, 0, namewstr.c_str(), -1, &namestr[0], len, nullptr, nullptr);
